...
class ModelBert4RecTime(tf.keras.models.Model):
    def __init__(self, num_items, model_cfg):
        super(ModelBert4RecTime, self).__init__()
        ...
        self.mlp_proj_time_encoding = tf.keras.models.Sequential([
           tf.keras.layers.Dense(model_cfg.trf_dim, activation=tf.nn.gelu, kernel_initializer=tf.keras.initializers.TruncatedNormal(mean=0, stddev=self.std_init)),
    ...
    def call(self, inputs, training=True, return_features=False):
        x_seq_past, x_seq_type, x_seq_encoding, x_mask, x_seq_recency = inputs
        ...
        elif self.model_cfg.hybrid_mask:
            mask = x_mask * pad_mask[:, 0, 0, :, tf.newaxis]
        else:
            mask = pad_mask
        ###########
...
def build_model_bert4Rec_time(num_items, model_cfg):
    return ModelBert4RecTime(num_items, model_cfg)
